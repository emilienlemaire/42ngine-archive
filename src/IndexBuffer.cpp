...
void IndexBuffer::setData(GLuint size) {
    glBufferData(
...
void IndexBuffer::addSubData(const std::vector<GLubyte> &data, long offset) const {
    glBufferSubData(
            GL_ELEMENT_ARRAY_BUFFER,
            offset,
            data.size() * sizeof(GLubyte),
            &data[0]
            );
}
...
bool IndexBuffer::findVertex(std::map<Vertex, GLubyte> &map, Vertex& vertex, GLubyte &index) const {
